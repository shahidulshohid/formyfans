...
const CreateAccount = () => {
  ...
  const handlePlaceSelected = ({ place }) => {
    ...
    setFormValues((prev) => ({ ...prev, location: locationObj }));
  ...
  const createAccount = async () => {
    ...
    if (formValues.isAdult === false) {
      return toast.error("You must confirm you are 18 or above");
    }
    ...
    setSignupData({
      ...formValues,
      location: normalizeLocationObject(formValues.location),
    });
  ...
  useEffect(() => {
    if (signupData) {
      setFormValues({
        ...
        location: normalizeLocationObject(signupData.location),
      });
    }
  }, [signupData]);
  ...
  return (
    <AuthContainer>
      ...
      <Grid container spacing={2} mb={2}>
        <Grid size={{ xs: 12 }}>
          <Label title="Gender" style={{ fontWeight: 400, fontSize: "16px" }} />
        ...
        <Grid size={{ xs: 12 }}>
          ...
          <GooglePlacesInput
            name="location_input"
            value={getLocationDisplayAddress(formValues.location)}
            onChange={(e) =>
              setFormValues((prev) => ({
                ...prev,
                location: {
      ...
      <Box mb={2}>
        <Label
          ...
          style={{ fontWeight: 400, fontSize: "16px" }}
        />
        <Typography fontSize={13} color="#8B92A4" mb={1.5}>
          Select at least 2 topics you are interested in
        </Typography>
        <Box display="flex" flexWrap="wrap" gap={1}>
          {INTERESTS.map((interest) => {
            const isSelected = formValues.interests.includes(interest.value);
      ...
      <FormControlLabel
        ...
        control={
          <Checkbox
            ...
            onChange={(e) =>
              setFormValues((prev) => ({ ...prev, isAdult: e.target.checked }))
            }
            sx={{ paddingTop: 0 }}
          />
        }
        label="This platform is strictly for users aged 18 and above. By signing up, you confirm that you are at least 18 years old. Any account found or suspected to belong to a person under 18 during the verification process may be suspended or permanently terminated."
      />
