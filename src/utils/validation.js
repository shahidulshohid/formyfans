...
import { calculateAge, hasValidLocation } from "./helper";
...
const createAccountValidation = (
  ...
  if (!obj.dob) {
    ...
    const dobDate = new Date(obj.dob);
    if (isNaN(dobDate.getTime())) {
      errors.dob = "Enter a valid date";
    } else if (dobDate > new Date()) {
      errors.dob = "Date of birth cannot be in the future";
    } else {
      const age = calculateAge(obj.dob);
      if (age < 18) {
  ...
  if (!hasValidLocation(obj.location)) {
    errors.location = "Country is required";
  }

  if (!Array.isArray(obj.interests) || obj.interests.length < 2) {
...
const updateProfileValidation = (obj, setInputError) => {
  ...
  if (!obj.dob) {
    ...
    if (isNaN(dobDate.getTime())) {
      ...
      const age = calculateAge(obj.dob);
      if (age < 18) {
        errors.dob = "You must be at least 18 years old to update your profile";
      }
    }
  }
  if (!obj.gender) {
...
const fanbugPaymentValidation = ({ amount, cardComplete }, setErrors) => {
  const errors = {};

  if (amount == null || Number.isNaN(Number(amount)) || Number(amount) <= 0) {
    errors.amount = "Please enter valid  fan bucks";
  }

  if (!cardComplete) {
    errors.card = "Please enter your card details";
  }
