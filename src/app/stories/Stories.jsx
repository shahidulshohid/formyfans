...
const formatSubheading = (author = {}, createdAt, collaborators = []) => {
  // collaborators can be an array of { user, userId } or plain user objects
  const username = author?.username ? `@${author.username}` : "@user";

  const createdDate = createdAt ? moment(createdAt) : null;
  const timeLabel = createdDate && createdDate.isValid() ? ` · ${createdDate.fromNow()}` : "";

  const coll = (collaborators || []).filter(Boolean).map((c) => c.user || c);
  if (!coll.length) return `${username}${timeLabel}`;

  const firstColl = coll[0];
  const collHandle = firstColl?.username ? `@${firstColl.username}` : firstColl?.firstName || firstColl?.name || "@user";

  if (coll.length === 1) {
    return `${username} and ${collHandle}${timeLabel}`;
  }

  return `${username} and ${coll.length} others${timeLabel}`;
};
...
export const Stories = () => {
  ...
  const mappedStories = useMemo(() => {
    ...
    return stories.map((story, index) => ({
      url: getFullS3Url(story.media),
      type: story.mediaType === "video" ? "video" : "image",
      muted: false,
      duration: story.mediaType === "video" ? 9000 : 5000,
      header: {
        heading: getDisplayName(author),
        subheading: formatSubheading(
          author,
          story.createdAt,
          stories[index]?.collaborators || [],
        ),
        profileImage: getFullS3Url(author.image) || "",
      },
    }));
  }, [currentGroup]);
