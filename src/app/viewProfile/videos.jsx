  ...
  const handleChangeFile = async (event) => {
    ...
    try {
      ...
      if (duration > MAX_VIDEO_DURATION_SECONDS) {
        toast.error("Video must be 1 hour or shorter.");
        return;
      }
    } catch {
          ...
          <Grid key={video._id} size={{ xs: 12, sm: 6, md: 3 }}>
            ...
            >
              {video.thumbnail ? (
                <Box
                  component="img"
                  src={getFullS3Url(video.thumbnail)}
                  alt="Video thumbnail"
                  loading="lazy"
                  sx={{
                    width: "100%",
      ...
      >
        ...
        {selectedVideo && (
          <Box
            component="video"
            ref={handleVideoRef}
            src={getFullS3Url(selectedVideo.fileName || selectedVideo.url)}
            controls
            autoPlay
            playsInline
