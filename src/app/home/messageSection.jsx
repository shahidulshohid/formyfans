...
import { DUMMY_POSTS } from "../../constants/dummyAuth";
...
const MessageSection = () => {
  ...
  const handleImageChange = async (event, type) => {
    ...
    try {
      ...
      if (type === "camera") {
        setFormData((prev) => ({
          ...prev,
          images: [{ mediaType, url: fileUrl?.url }],
          // TODO: Add camera media to the form data
          // images: [...prev.images, { mediaType, url: fileUrl?.url }],
        }));
      } else if (type === "gallery") {
        setFormData((prev) => ({
          ...prev,
          images: [{ mediaType, url: fileUrl?.url }],
          // TODO: Add multiple gallery media to the form data
          // images: [...prev.images, { mediaType, url: fileUrl?.url }],
        }));
      } else if (type === "gif") {
        setFormData((prev) => ({
          ...
          images: [{ mediaType: "gif", url: fileUrl?.url }],
          // TODO: Add gif image to the form data
          // images: [...prev.images, { mediaType: "gif", url: fileUrl?.url }],
        }));
      }
    } catch {
  ...
  const handleCreatePost = async () => {
    ...
    try {
      ...
      try {
        ...
        // Backend offline fallback - simulate local post creation
      }
      ...
      const mockNewPost = {
        ...
        author: {
          _id: user?._id || "65f1a2b3c4d5e6f7a8b9c0d1",
          firstName: user?.firstName || "Shahidul",
          lastName: user?.lastName || "Islam",
          username: user?.username || "shahidul_demo",
      ...
      toast.success(editingPostId ? "Post updated!" : "Post published successfully!");
      setFormData({ post: "", images: [] });
      setPostList((prev) => [mockNewPost, ...prev]);
      setEditingPostId(null);
    } catch {
  ...
  const handleGetPosts = async (page = 1) => {
    ...
    try {
      ...
      // Backend offline: keep DUMMY_POSTS
    } finally {
      page === 1
