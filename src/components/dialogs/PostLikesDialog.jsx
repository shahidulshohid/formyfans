...
import { getCampaignLikes } from "../../api/modules/campaign";
...
export const PostLikesDialog = forwardRef((_, ref) => {
  ...
  const handleGetLikes = async (itemId, page = 1) => {
    page === 1
      ? setIsLoading((prev) => ({ ...prev, getLikes: true }))
      : setIsLoading((prev) => ({ ...prev, loadMoreLikes: true }));
    try {
      const isCampaign = data?.data?.itemType === "campaign";
      const response = isCampaign
        ? await getCampaignLikes(itemId, { page, limit: 10 })
        : await getPostLikes(itemId, { page, limit: 10 });
