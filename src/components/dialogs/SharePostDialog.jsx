...
import { shareCampaign } from "../../api/modules/campaign";
...
export const SharePostDialog = forwardRef(({ setPostList }, ref) => {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      if (response?.status === 200 || response?.status === 201) {
        ...
        toast.error(
          response?.data?.message ||
            (isCampaign ? "Could not share campaign" : "Could not share post"),
        );
      }
    } catch (error) {
      console.error(error);
  ...
  return (
    <DialogBox open={open} onClose={handleClose} maxWidth="sm" fullWidth>
      <DialogHeader
        title={
          data?.data?.itemType === "campaign" ? "Share Campaign" : "Share Post"
        }
        onClose={handleClose}
      />
