...
import { PostAuthorHeading } from "../cards";

export const PostDetailDialog = forwardRef(({ setPostList }, ref) => {
  ...
  const handleCreateComment = useCallback(async () => {
    ...
    try {
      ...
      if (commentMentions.length > 0) {
        payload.mentions = commentMentions;
      }
      const response = await createComment(post._id, payload);
      setCommentMentions([]);
      if (response.status === 200 || response.status === 201) {
  ...
  }, [commentInput, commentMentions, post?._id, setPostList]);

  // Collaborative posts: collaborators is an array of { userId, user }.
  // `user` can be null/absent if it wasn't populated - filter those out.
          ...
          >
            ...
            >
              <Box
                ...
                to={
                  author?.username ? `/${author.username}/all-posts` : undefined
                }
                onClick={handleClose}
                sx={{
                  textDecoration: "none",
