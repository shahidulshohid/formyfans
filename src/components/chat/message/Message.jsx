...
import { CampaignDetailDialog, PostDetailDialog } from "../../dialogs";
import { SharedDealPreview } from "./Shareddealpreview";
import { acceptDeal, rejectDeal } from "../../../api/modules/deal";
...
const SharedCampaignPreview = ({ sharedCampaign, campaignDetailDialogRef }) => {
  if (!sharedCampaign?.campaignId && !sharedCampaign?.brand) return null;

  const populated =
    typeof sharedCampaign.campaignId === "object" &&
    sharedCampaign.campaignId !== null
      ? sharedCampaign.campaignId
      : null;

  const campaignId =
    populated?._id ||
    (typeof sharedCampaign.campaignId === "string"
      ? sharedCampaign.campaignId
      : sharedCampaign.campaignId?.toString?.());

  const brand = populated?.brand || sharedCampaign.brand || "Sponsored";
  const headline =
    populated?.advertisement?.headline ||
    sharedCampaign.advertisement?.headline ||
    populated?.name ||
    sharedCampaign.name ||
    "Campaign";
  const primaryText =
    populated?.advertisement?.primaryText ||
    sharedCampaign.advertisement?.primaryText ||
    populated?.description ||
    sharedCampaign.description ||
    "";
  const media =
    populated?.advertisement?.media?.[0] ||
    sharedCampaign.advertisement?.media?.[0];
  const isVideo =
    media?.mediaType === "video" || media?.url?.includes("/video/upload/");

  const handleOpenCampaign = () => {
    if (!campaignId) return;
    campaignDetailDialogRef.current?.open({
      _id: campaignId,
      itemType: "campaign",
      brand,
      name: populated?.name || sharedCampaign.name,
      description: populated?.description || sharedCampaign.description,
      advertisement: populated?.advertisement || sharedCampaign.advertisement,
      endDateTime: populated?.endDateTime || sharedCampaign.endDateTime,
      startDateTime: populated?.startDateTime,
      likesCount: populated?.likesCount ?? 0,
      commentsCount: populated?.commentsCount ?? 0,
      sharesCount: populated?.sharesCount ?? 0,
      isLiked: false,
    });
  };
    ...
    <Box sx={{ mb: 0.5, width: "100%", maxWidth: 260 }}>
      ...
      >
        ...
        <Box px={1.25} py={0.85}>
          ...
          <Typography fontSize={12} color="#8B92A4" noWrap>
            {headline}
          </Typography>
        </Box>
        ...
        {primaryText && (
          <Typography
            ...
            sx={{
              ...
              WebkitBoxOrient: "vertical",
              overflow: "hidden",
              cursor: campaignId ? "pointer" : "default",
            }}
          >
            {primaryText}
          </Typography>
...
const MessageBody = ({ socket }) => {
  ...
  useEffect(() => {
    ...
    const handleDealStatusUpdated = (payload) => {
      if (payload.conversationId !== activeChat?.conversationId) return;
      updateMessageSharedDealStatus?.(payload.dealId, payload.status);
    };
    socket.on("deal_status_updated", handleDealStatusUpdated);
    return () => socket.off("deal_status_updated", handleDealStatusUpdated);
  }, [socket, activeChat]);
  ...
  const handleAcceptDeal = async (dealId) => {
    try {
      setDealActionLoading((prev) => ({ ...prev, accept: true }));
      const response = await acceptDeal(dealId);
      if (response.data.status === "success") {
        // updateMessageSharedDealStatus?.(dealId, "accepted");
        toast.success(
          response.data.message || "Deal status accepted successfully",
        );
      } else {
        toast.error(response.data.message || "Failed to accept deal");
      }
    } catch (err) {
      toast.error(err?.response?.data?.message || "Failed to accept deal");
    } finally {
      setDealActionLoading((prev) => ({ ...prev, accept: false }));
    }
  };

  const handleRejectDeal = async (dealId) => {
    try {
      setDealActionLoading((prev) => ({ ...prev, reject: true }));
      const response = await rejectDeal(dealId, {
        reason: "rejected by the receiver",
      });
      if (response.data.status === "success") {
        // updateMessageSharedDealStatus?.(dealId, "rejected");
        toast.success(
          response.data.message || "Deal status rejected successfully",
        );
      } else {
        toast.error(response.data.message || "Failed to reject deal");
      }
    } catch (err) {
  ...
  return (
    <Box flex={1} overflow={"auto"} p={1.5}>
      {loading ? (
        ...
        <Box display="flex" flexDirection="column" gap={1}>
          {insideMessages.map((message, index) => {
            ...
            const hasSharedDeal = Boolean(message.sharedDeal);
            const hasSharedCampaign = Boolean(message.sharedCampaign);
            const hasSharedContent =
              hasSharedPost || hasSharedDeal || hasSharedCampaign;
            return (
              <Box
                key={message._id}
                display="flex"
              ...
              >
                ...
                >
                  ...
                  {/* Message Bubble */}
                  <Box
                    px={hasSharedContent ? 0.75 : isAudioMsg ? 1 : 1.5}
                    py={hasSharedContent ? 0.75 : 1}
                  ...
                  >
                    {isAttachment && (
                      <>
                        ...
                        {isImageMsg && (
                          ...
                          >
                            <img
                              src={getFullS3Url(message.attachment.url)}
                        ...
                        {isAudioMsg && (
                          <AudioBubble
                            url={getFullS3Url(message.attachment.url)}
                            isOwn={isOwn}
                          />
                        )}
                      </>
                    ...
                    {hasSharedPost && (
                      <SharedPostPreview
                        ...
                        postDetailDialogRef={postDetailDialogRef}
                      />
                    )}
                    {hasSharedCampaign && (
                      <SharedCampaignPreview
                        sharedCampaign={message.sharedCampaign}
                        campaignDetailDialogRef={campaignDetailDialogRef}
                      />
                    )}
                    {hasSharedDeal && (
                      <SharedDealPreview
                        sharedDeal={message.sharedDeal}
                        isOwn={isOwn}
                        onAcceptDeal={handleAcceptDeal}
                        onRejectDeal={handleRejectDeal}
                        dealActionLoading={dealActionLoading}
                      />
                    )}
      ...
      <CampaignDetailDialog ref={campaignDetailDialogRef} />
    </Box>
  );
};
